# -*- coding: utf-8 -*-
"""Plot the reinforcement of a contrast sweep against the structure PHR.

The transpose of ``plot_reinforcement_vs_contrast.py``: x = PHR,
y = reinforcement, one colour per filler/matrix contrast.
...
Usage:
    python Plotting_scripts/plot_reinforcement_vs_phr.py
    python Plotting_scripts/plot_reinforcement_vs_phr.py --strain 0.1
    python Plotting_scripts/plot_reinforcement_vs_phr.py --strain common --show
    python Plotting_scripts/plot_reinforcement_vs_phr.py <sweep_dir> --raw-p11
"""
...
def make_plot(rows, strain, args):
    ...
    for index, contrast in enumerate(contrasts):
        series = sorted(
            (row for row in rows if row["contrast"] == contrast),
            key=lambda row: row["phr"],
        )
        color = _COLORS[index % len(_COLORS)]
        marker = _MARKERS[index % len(_MARKERS)]
        phr = [row["phr"] for row in series]
        y = [row[y_key] for row in series]
        if args.lines:
            ax.plot(phr, y, color=color, linewidth=1.0, alpha=0.35, zorder=1)
        ax.scatter(
            phr, y, s=args.marker_size, marker=marker,
    ...
    ax.set_title(args.title or "Reinforcement vs PHR at strain {:g} "
...
def print_table(rows, skipped, strain):
    ...
    print("\nneat-matrix reference: model {}, E = {:g}, nu = {:g}".format(
        model, young, poisson))
    print("P11_matrix(F11 = {:g}) = {:.6f}".format(1.0 + strain, rows[0]["matrix_p11"]))

    contrasts = sorted({row["contrast"] for row in rows})
    phrs = sorted({row["phr"] for row in rows})
    lookup = {(row["contrast"], row["phr"]): row for row in rows}
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot reinforcement vs PHR, coloured by contrast."
    )
    ...
    parser.add_argument("--out", default=None,
                        help="Output PNG (default: reinforcement_vs_phr.png in sweep_dir).")
    ...
    args.out = args.out or os.path.join(
        args.sweep_dir, "reinforcement_vs_phr{}.png".format(suffix))
