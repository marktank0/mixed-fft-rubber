...
def test_solver(out_root, structure, charge, N, incre):
    ...
    def run(tag, module, **kw):
        ...
        prob = module.FFTSolver(structure, charge_path=charge, output_path=path,
                                N=N, output_name=".")
        t0 = time.time()
        # the pinned baseline solver predates both changes: it still calls
        # the Green preconditioner "reference", and still needs savemodel to
        # write output.csv (the live solver always saves)
